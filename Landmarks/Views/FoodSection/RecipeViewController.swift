...
struct MealListView: View {
    ...
    let diets = ["vegetarian", "vegan", "paleo"]
            ...
            if modeldata.meals.isEmpty {
                Form {
                    Section(header: Text("Meal Preferences")) {
                        Stepper("Target Calories: \(targetCalories)", value: $targetCalories, in: 1000...4000, step: 100)
                        Picker("Diet", selection: $diet) {
                            ForEach(diets, id: \.self) {
                                Text($0)
                            }
                        }
                        TextField("Excluded Ingredients", text: $exclude)
    ...
    func load_daily_meals_plan() {
        ...
        let requestUrl = "https://spoonacular-recipe-food-nutrition-v1.p.rapidapi.com/recipes/mealplans/generate?timeFrame=day&targetCalories=\(targetCalories)&diet=\(diet)&exclude=\(exclude.replacingOccurrences(of: " ", with: "%20"))"
                           ...
                           for mealDict in mealsArray {
                               if let id = mealDict["id"] as? Int,
                                  ...
                                  let readyInMinutes = mealDict["readyInMinutes"] as? Int {
                                   ...
                                   meals.append(meal)
                                   let newCalories = Double(calories) / (3.0 * Double(rec_serving))
                                   let newFoodSchedule = FoodSchedule(food: meal, calories: newCalories, servings: Int(rec_serving), date: Date(), selectedTime: "", isSelected: false, mealType: 0)
                                   modeldata.mealSchedule.append(newFoodSchedule)
                               }
                           }
                       }
                                       } catch let error {
